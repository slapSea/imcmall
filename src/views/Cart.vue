    ...
    <div>
            ...
            <div class="cart-foot-inner">
              <div class="cart-foot-l">
                <div class="item-all-check">
                  ...
                  <span class="checkbox-btn item-check-btn" v-bind:class="{'check': checkAllClick}">
              ...
              <div class="cart-foot-r">
                ...
                <div class="btn-wrap">
                  <a class="btn btn--red" v-bind:class="{'btn--dis':countCheckNum==0}" @click="checkOut">Checkout</a>
                </div>
              </div>
            </div>
          </div>
      ...
      <Modal v-bind:mdShow="mdShowCart" v-on:close="closeModal">
        ...
        <div slot="btnGroup">
          ...
          <a class="btn btn--m" href="javascript:;" @click="mdShowCart=false">取消</a>
        </div>
      </Modal>
...
<script>
    ...
    export default {
        ...
        computed:{
          checkAllClick(){
            return this.countCheckNum==this.cartList.length
          },
          countCheckNum(){
            let i = 0
        ...
        methods:{
          ...
          editCart(flag,item){
            ...
            axios.post("/users/editCart",param).then(response=>{
              let res = response.data
              if(res.status === "0"){
                console.log("success")
              }
          ...
          toggleAll(){
            let flag = !this.checkAllClick
            this.cartList.forEach((item)=> {
              item.checked = flag
            })

            axios.post("/users/checkAllClick",{"checkAllClick":flag}).then((response)=>{
              let res = response.data
              if(res.status=="0"){
                console("checkAllClick success!")
              }
            }).catch(function (error) {
              console.log(error.response)
            })
